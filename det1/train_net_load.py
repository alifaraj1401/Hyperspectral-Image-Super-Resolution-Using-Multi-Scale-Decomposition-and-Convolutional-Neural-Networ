...
def train():
    # load the model
    new_model = load_model("checkpointdet1 (50).h5")
    ...
    filepath = 'D:/test/arch3to3scale2\pavia\det1\save/checkpointapp-{epoch:02d}-{loss:.2f}.h5'
    ...
    for epoch in range(0, n_epoch,n_epoch):
        H = new_model.fit(X, y, batch_size=16, nb_epoch=n_epoch, shuffle='batch',callbacks=callbacks_list)
        if args.save:
            print("Saving model ", epoch + n_epoch)
            new_model.save(os.path.join(args.save, 'app_%d.h5' %(epoch+n_epoch)))
